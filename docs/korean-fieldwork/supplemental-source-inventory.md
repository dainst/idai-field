...
조사방법론 강의자료의 공통 반영점은 유적 유형별 화면보다 먼저 적용한다. 개인 야장은 `개인야장공적기록화`로 제출·검토·열람 상태를 관리하고, `기록생성시점관리`로 현장시에만 가능한 기록과 보고서 단계 생성 기록을 구분한다. 원고·도면·사진·목록·기준토층·번호 변환표는 `보고서편집교차검토`로 묶고, 보고서에 들어가지 않은 원도면·원사진·전자야장 원자료도 `디지털원자료보존`으로 백업·열람 절차를 남긴다. 이 공통 축은 1차로 `KoreanFieldwork-recordCreationTiming`, `KoreanFieldwork-verificationState`, `KoreanFieldwork-fieldRecordQuality`, `KoreanFieldwork-personalNotebookArchive`, `KoreanFieldwork-digitalSourcePreservation`, `KoreanFieldwork-termRelation`, `KoreanFieldwork-featurePackage`, `KoreanFieldwork-samplePurpose`, `KoreanFieldwork-reportCrossCheck` 값 목록에 옮겼다.
