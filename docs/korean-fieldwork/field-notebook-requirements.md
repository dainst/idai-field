...
구석기 조사 보강분은 `Survey`에 `paleolithicSurveyStage`, `paleolithicLocationSource`, `paleolithicFieldCollection`, `paleolithicTrialPitCoordinateControl`, `paleolithicProfileSampleRecord`, `paleolithicNonSiteResourceSurvey`를 추가했다. 표면에서 정형 석기가 확인되지 않아도 단구·고토양·기존 신고지점·지도자료, 부착토와 토양시료, 시굴 피트 좌표 유지, 단면 시료, 주변 돌감 분포가 함께 남아야 후속 정밀발굴과 전문검토로 이어질 수 있다. 구석기 꼬리표·유물대장 요구는 모든 출토유물에 적용되는 `Find`의 `artifactLabelRegisterLink`로 이미 승격했으므로, 이번 묶음은 조사단계와 입지·층위·시료·비유적지 자원 기록에 집중한다.
