...
export const canReviseKoreanFieldworkIdentifier = (
  document: Document
): boolean => IDENTIFIER_REVISION_CATEGORIES.has(document.resource.category);

export const getKoreanFieldworkFieldIdentifier = (
  document: Document
): string => {
...
export const getKoreanFieldworkReportIdentifier = (
  document: Document
): string => {
  const resource = getResource(document);
...
const isIdentifierRevision = (
  value: unknown
): value is KoreanFieldworkIdentifierRevision => {
