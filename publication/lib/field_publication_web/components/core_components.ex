defmodule FieldPublicationWeb.CoreComponents do
  @moduledoc """
  ...
  def input(%{type: "select"} = assigns) do
    ...
    <div phx-feedback-for={@name}>
      ...
      <select
        id={@id}
        name={@name}
        class="mt-2 block w-full rounded-md rounded-b-none border border-gray-300 bg-white shadow-sm focus:border-zinc-400 focus:ring-0 sm:text-sm"
        multiple={@multiple}
        {@rest}
      >
    ...
    <div class="mt-14">
      <dl class="-my-4 divide-y divide-zinc-100">
        <div :for={item <- @item} class="flex gap-4 py-4 text-sm leading-6 sm:gap-8">
          <dt class="w-2/12 flex-none text-zinc-500"><%= item.title %></dt>
          <dd class="w-10/12 text-zinc-700"><%= render_slot(item) %></dd>
        </div>
      </dl>
    </div>
    """
