...
| 보고서 역산 | 보고서 초록, 도면 목록, 사진대장, 유물·시료 목록은 나중에 따로 만드는 문서가 아니라 현장기록에서 생성되어야 한다. | `ReportPreparationReview`, `ReportStandardHistory`, `ReportEditorialCrossCheck`가 원고, 도면, 사진, 일지, 목록, 기준 변경, 번호 변환표의 불일치를 원기록 ID와 연결한다. |
