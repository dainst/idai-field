import { getKoreanFieldworkEvidenceChips } from './korean-fieldwork-record-evidence';
import { KOREAN_FIELDWORK_CATEGORIES } from './korean-fieldwork-categories';

const C = KOREAN_FIELDWORK_CATEGORIES;

describe('Korean fieldwork record evidence', () => {
  it('summarizes field evidence attached to a feature record', () => {
    const feature = createDoc('feature-1', C.FEATURE, '수혈 1');
    const chips = getKoreanFieldworkEvidenceChips(feature, [
      feature,
      createDoc('pit-1', C.FEATURE_SEGMENT, '피트 1', {
        liesWithin: ['feature-1'],
      }),
      createDoc('layer-1', C.LAYER, '층위 1', {
        liesWithin: ['feature-1'],
      }),
      createDoc('photo-1', C.PHOTO, '사진 1', {
        depicts: ['feature-1'],
      }),
      createDoc('sample-1', C.SAMPLE, '시료 1', {
        liesWithin: ['feature-1'],
      }),
    ] as any);

    expect(chips).toEqual([
      { id: 'featureSegments', label: '피트', count: 1, tone: 'filled' },
      { id: 'layers', label: '층위', count: 1, tone: 'filled' },
      { id: 'photos', label: '사진', count: 1, tone: 'filled' },
      { id: 'soilProfilePhotos', label: '토층', count: 0, tone: 'empty' },
      { id: 'finds', label: '유물', count: 0, tone: 'empty' },
      { id: 'samples', label: '시료', count: 1, tone: 'filled' },
    ]);
  });

  it('keeps non-structural evidence records compact in the record list', () => {
    expect(getKoreanFieldworkEvidenceChips(
      createDoc('photo-1', C.PHOTO, '사진 1'),
      []
    )).toEqual([]);
  });
});

const createDoc = (
  id: string,
  category: string,
  identifier: string,
  relations: Record<string, string[]> = {}
) => ({
  resource: {
    id,
    identifier,
    category,
    relations,
  },
});
