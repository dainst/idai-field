...
| 우연·신고 유물 출처 | 주민 신고, 공사·경작 중 발견, 지표 채집, 기존 소장·보고서 자료처럼 원위치가 흔들릴 수 있는 유물은 일반 발굴 출토품처럼 해석하면 안 된다. | `Find`의 `chanceFindProvenance`가 신고자·발견 활동, 정확한 출토위치 불명, 조사 전 훼손, 원맥락 파괴, 구술 신고만 있음, 지도 위치 개략, 후속 확인조사 필요, 일반 발굴맥락 아님을 유물 카드에서 표시한다. |
...
| 유물 수량과 수장 기준 | 국가귀속·이관용 수량은 파편 수가 아니라 행정 건수, 물리 점수, 동일개체·부속구 판단, 실수량 병기가 결합된 값이다. 수장고 관리는 표준 온습도 수치만이 아니라 개별 포장, 온습도 피크와 결로, 재질별 미세환경, 이관 동선, 대형유물 장비·인력까지 포함한다. | `artifactQuantityBasis`가 `건/점/실수량/구성품/근거사진`을 분리하고, `storageEnvironmentControl`이 재질별 온습도·조도·오염물 기준, 실측값, 결로 방지, 이관 순서별 구획, 입구 포장테이블, 대형유물 지지장치와 추가 인력을 저장한다. |
...
16. 보존과학 기록이 실내 후처리로 밀리지 않고 노출·수습·임시보관·분석 의뢰·파괴분석 승인·처리 원칙을 현장 단계에서 추적하며, 수장고 보관은 개별 포장, 결로 방지, 재질별 미세환경, 이관 동선, 대형유물 장비·인력까지 남기는가.
...
31. 원문 도표·사진·표·캡션·사례명·한자·수치가 값목록으로 바로 승격되지 않고, `SourceEvidenceIndex`에서 적용 영역, 대조 상태, 사용 목적과 함께 추적되는가.
32. 측량·사진·도면·안전·민원·유물정리·시료·일지·보고서 책임과 검토자가 일지 문장 속에 묻히지 않고 `operationRoleResponsibility`로 구조화되는가.
33. 허가서 비치, 보안, 주민·관할기관 통보, 유적보호 주의사항, 기상 대비, 야간방범, 임시유물·기록물 보관이 행정 메모가 아니라 `Operation`의 `siteProtectionSecurity`로 남는가.
34. 구획·둑·배수·장비 제토·제토량·굴삭기 조합·표토량·적토/반출·기준단면·안전휀스·철수 전 기록물 점검이 일지 문장 속에 묻히지 않고 `Operation`의 `excavationControlSafety`로 검수되는가.
35. 발굴맥락이 유구명 하나로 닫히지 않고 유적·유구·유물·유물복합체·환경 연결, 수평분포·수직층위, 조사방법 선택근거와 함께 남는가.
36. 표토·현대교란·도굴갱·상부퇴적토·유구 매몰토 제거와 축조면·사용면·폐기면 확인이 행위의 역순 원칙으로 검토되는가.
37. 유구·유물 해석이 연구질문, 관찰·해석 분리, 대안해석, 자연·후퇴적 가능성, 분석결과 영향, 보고서 채택 근거와 함께 남는가.
38. 건물지 판단이 지상식·고상식 같은 복원명으로 닫히지 않고 주혈군 측정·촬영, 한 칸 바깥 범위, 초석 설치·발취 흔적, 굴립주·독립기초 구분, 보존 결정을 함께 남기는가.
39. 건물지 복원안이 전문가 검토, 영조척 계산, GPR·3D 후처리, 보존·활용 결정과 분리되어 검증되는가.
40. 수혈건물지 조사가 평면확인, 토층둑 배치, 1~3차 노출, 동일 촬영지점·드론 경로, 외부시설 조사와 해체 후 최종평면으로 이어지는가.
