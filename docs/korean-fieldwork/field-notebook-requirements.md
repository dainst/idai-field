...
`TermImportMapping`은 `TermAuthority` 하위 카드로 두어 기존 엑셀, 유물대장, 사진 캡션, 보고서 목록의 입력어를 권위 표제어와 보고서 출력어에 연결한다. 이 카드는 자동 치환 규칙이 아니라 원 입력어 보존, 권위어 연결, 보고서 출력어 분리, 분야 범위, 수동 검토 필요 여부를 함께 남기는 가져오기 검토 기록이다.
