defmodule Api.Worker.Images.ImageMagickImageConverter do
  ...
  * and shell commands
  ...
  def environment_ready() do
    ...
    status == 0 and
      required_version_matches(version, @required_imagemagick_version) and
      required_delegates_present(delegates)
  end
  ...
  def convert_files(project) do
    ...
    # files (and folders) in sources dir
    {:ok, files_and_folders} = File.ls(sources_dir)

    files_and_folders
    # without file extension
    |> Enum.filter(&(not String.contains?(&1, ".")))
    # only files
    |> Enum.filter(fn file -> not File.dir?(Path.join(sources_dir, file)) end)
  ...
  def crop(
        ...
        rescale,
        z_index,
        %{x_index: x_index, y_index: y_index, x_pos: x_pos, y_pos: y_pos}
      ) do
    x_folder =
      Path.join([
        @imageroot,
    ...
    if File.exists?(target_img_path) do
      ...
      {cmd, args} = {
        ...
        [
          source_img_path,
          # suppress 'convert: geometry does not contain image', some tiles simply will have no content with tiled rectangular images
          "-quiet",
          "-crop",
  ...
  defp get_im_version_and_delegates() do
    ...
    [version] =
      Enum.filter(String.split(version), fn part -> String.match?(part, ~r/\d*\.\d\.\d/) end)
