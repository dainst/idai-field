...
export default class PreviewOverlay {
  ...
  constructor(
    ...
    const _this = this;
    this.map.getTargetElement().addEventListener("pointerleave", function (e) {
  ...
  private renderPreviewIcon(feature: FeatureLike) {
    ...
    const categoryMetadata = this.categoriesMetadata.find(
      (elem) => elem.category == properties.category,
    );

    categoryLabel.appendChild(
      document.createTextNode(
        `${pickTranslation(categoryMetadata.category_label, this.language)}`,
      ),
    );
