...
보고서 제출·작성 전 검토·국가귀속 보강분은 `ReportSubmissionWorkflow`, `ReportPreparationReview`, `StateVestingSelectionRecord`를 `Project` 하위 카드로 두었다. 완료신고, 완료자료, 약식보고서, 최종보고서 PDF, 전자행정 접수번호, 초록·협업포털 입력, 원기록 연결을 제출 흐름으로 관리하고, 최종보고서 편집 전에는 일러두기, 허가번호, 방위, 축척·축소율, 해발고도, 토색, 도면기호, 유물 관찰·실측 기준, 원야장·원도면·원사진·대장류·번호 변환표와 도면·사진 일치 여부를 별도 검수한다. 소유권 공고, 선별회의록, 임시보관증, 보관관리기관, 국가귀속대장, 보존처리 후 정정·재평가는 프로젝트 체크값에 묻지 않고 증빙 단위로 추적한다.
