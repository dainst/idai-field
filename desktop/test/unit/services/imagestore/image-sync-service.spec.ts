...
describe('ImageSyncService', () => {
  ...
  it('remotely added images are evaluated correctly by diff function', async done => {
    ...
    try {
      ...
      await imageStoreExpressServer.store('0', mockImage, testProjectName, ImageVariant.ORIGINAL);
      await imageStoreExpressServer.store('0', mockImage, testProjectName, ImageVariant.THUMBNAIL);
  ...
  it('locally deleted images are evaluated correctly by diff function', async done => {
    ...
    try {
      ...
      expect(diff.deleteRemotely.includes('0')).toBe(true);
      done();
    } catch (err) {
      fail(err);
    }
  ...
  it('remotely deleted images are evaluated correctly by diff function', async done => {
    ...
    try {
      ...
      await imageStoreExpressServer.store('0', mockImage, testProjectName, ImageVariant.ORIGINAL);
      await imageStoreExpressServer.store('0', mockImage, testProjectName, ImageVariant.THUMBNAIL);
      await imageStoreExpressServer.remove('0');
      ...
      expect(diff.deleteLocally.includes('0')).toBe(true);
      done();
    } catch (err) {
      fail(err);
    }
