...
const DatingField: React.FC<FieldBaseProps> = ({
  field,
  setFunction,
  currentValue,
}) => {
  ...
  const submitHandler = () => {
    ...
    Object.keys(dating).forEach(
      (key) => dating[key] === undefined && delete dating[key]
    );
  ...
  const removeBtnHandler = (index: number) => {
    if (currentValue && Array.isArray(currentValue))
      setFunction(
        field.name,
        (currentValue as Dating[]).filter((_dating, i) => i !== index)
      );
  };
  ...
  const renderItem = ({ item, index }: { item: Dating; index: number }) => (
    <Row style={styles.currentValues} testID={`currentValueDating_${index}`}>
      ...
      <Button
        ...
        icon={
          <Ionicons name="trash" size={12} testID={`datingRemove_${index}`} />
        }
      />
    </Row>
  );
        ...
        <View testID="datingForm">
          ...
          {type === 'range' && (
            <PeriodForm
              ...
              setIsImprecise={setIsImprecise}
              isUncertian={isUncertain}
              setIsUncertian={setIsUncertain}
              source={source}
              setSource={setSource}
