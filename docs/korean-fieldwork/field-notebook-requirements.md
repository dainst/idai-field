...
취락 단위 공간해석 보강분은 `FeatureGroup`에 `settlementSpatialInterpretation`을 추가했다. 집자리 중복관계, 동시기 집자리, 한 시점 규모, 지붕·벽체·화덕 근거, 행위공간 구획, 공동이용공간과 세대별 공간 후보, 폐기 방식, 바닥 유물분포, 식물유체와 잔류물·내용물 분석 연결, 반복 축조 원인 검토를 취락 카드에서 함께 남겨 개별 유구 기록이 마을 해석으로 넘어가는 근거를 보존한다.
