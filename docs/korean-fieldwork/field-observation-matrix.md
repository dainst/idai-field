...
| 자연과학 시료 | 분석 목적, 적정 방법, 파괴 여부, 채취 위치, 채취량, 포장, 냉장/냉동, 오염 위험, 시료 대표성, 전처리, Lab No., 측정값, 역년 보정, 결과 채택 여부 | `analysisPurpose`, `methodCandidate`, `samplingContext`, `sampleAmount`, `contaminationRisk`, `storageTemperature`, `sampleRepresentativeness`, `pretreatment`, `labNumber`, `calibratedAge`, `resultStatus` | 분석 대상과 목적에 맞는 방법을 고르고, 결과의 한계까지 검토해야 한다. 절대연대 시료는 어떤 고고학적 시점을 묻는지, 시료가 그 시점을 대표하는지, 보정과 이상값 처리를 어떻게 했는지가 핵심이다. |
