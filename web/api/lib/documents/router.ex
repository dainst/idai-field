defmodule Api.Documents.Router do
  ...
  get "/:id" do
    with doc = %{project: project, resource: resource} <- Index.get(id),
         ...
         layouted_doc <-
           put_in(doc.resource, to_layouted_resource(config, resource, project_resource)) do
      send_json(conn, layouted_doc, [cache_header()])
    else
  ...
  get "/predecessors/:id" do
    with doc = %{project: project} <- Index.get(id),
         :ok <- access_for_project_allowed(conn.private[:readable_projects], project) do
      send_json(conn, %{results: Predecessors.get(doc)}, [cache_header()])
    else
      nil -> send_not_found(conn)
      :unauthorized_access -> send_unauthorized(conn)
      _ -> IO.puts "other error"
  ...
  get "/descendantsImages/:id/:numberOfImages" do
    with doc = %{project: project} <- Index.get(id),
      ...
      send_json(
        ...
        %{
          results:
            DescendantsImages.get(
              doc,
              elem(Integer.parse(numberOfImages), 0),
              conn.private[:readable_projects]
            )
        },
        [cache_header()]
