...
| 🖥️ 데스크톱 검토 | 현장 기록을 SHP/DXF/CAD, 보고서 대장, 근거자료 묶음과 맞춰 점검 |
...
6. 데스크톱에서 드론사진, 정식 측량, CAD, 보고서 자료와 맞춰 geometry와 근거자료 연결을 검토합니다.

## 🧩 현재 구현된 방향

### 🇰🇷 한국 현장 설정

`KoreanFieldwork` 설정과 `디지털 야장` 시작 템플릿을 제공합니다. 프로젝트 생성 시 한국어를 기본 언어로 쓰고, 현장 기록에 필요한 범주와 값목록을 우선 노출합니다.
...
데스크톱은 현장 입력기를 대체하지 않고, 현장 기록을 정리하고 보고서로 이어지는 근거자료 흐름을 확인하는 쪽에 둡니다.
...
Windows에서 디지털 야장을 바로 실행하기 위한 보조 스크립트:
