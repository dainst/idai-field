...
| 용어 권위와 검색 분리 | 전문사전 표제어, 기관 관용어, 한자·원어 색인은 모두 필요하지만 저장값으로 바로 합치면 안 된다. 일반 사전의 유적명·수치·색인 원칙도 현장 입력 강제가 아니라 보고서 출력 규칙으로 추적해야 한다. | `TermAuthority`, `TermAlias`, `TermRelationship`을 두고 검색어·보고서출력어·우선표제어·검증상태를 분리한다. `dictionaryEditorialRule`은 한글 표제어, 한자·원어 병기, 유적명 정규화, 유구/유물 단위 출력, 연대·수량 표기, 색인 연결을 표제어 후보에서 따로 관리한다. |
