...
현재 iDAI.field 설정 모델에서 `mandatory`는 필드 전체에 일괄 적용되는 값이고, 선택값에 따라 달라지는 조건부 필수값은 `missingMandatoryFields` 경고 흐름에 직접 연결되지 않는다. 따라서 한국형 야장에서는 조건부 필수처럼 보이는 요구를 무리하게 필드 단위 강제로 넣기보다, `fieldOnlyMissingCheck`, `firstExposureRecord`, 품질검수 카드, 원문 근거 색인처럼 현장 복구 불가능성과 검토 상태를 드러내는 필수 체크리스트로 먼저 다룬다.
...
| 현장기록 품질검수 | 좋은 야장은 개인 메모가 아니라 즉시성, 사실/해석 분리, 교차검토가 가능한 공적 기록이다. 현장에서 휴대하며 방안 약식실측을 남기고, 같은 유형의 유구라도 각각을 별개 질문으로 보고, 보고서 미수록 현장관찰과 조사자의 판단변경까지 보고서 기술로 이어져야 한다. | `FieldRecordQualityReview`를 두고 작성자, 담당 유구, 작성시점, 검토자, 사실/해석 분리, 수정 근거를 저장한다. `fieldRecordQuality`는 유구별 서사 기록, 현장-보고서 연속성, 조사자 보고서기술 연결을 함께 점검하고, `personalNotebookArchive`는 휴대용 야장, 방안 약식실측, 시행착오·판단변경, 보고서 미수록 관찰, 원본 제출과 열람조건을 관리한다. |
...
| 제토·둑·안전 운영 | 구획·둑 설정, 배수, 장비 제토, 기준단면 촬영과 보호, 안전거리, 철수 전 기록 점검은 유구 유형보다 먼저 작업 단위에서 관리되어야 한다. 제토는 유구조사 시간을 확보하는 운영 판단이므로 장비·인부별 일일 제토량, 굴삭기 규격 조합, 표토 두께와 토량, 적토장·반출 계획도 같이 남긴다. | `Operation`의 `excavationControlSafety`가 둑 폭·보호, 배수로·집수정, 장비 제토 두께, 제토량·장비조합, 표토량·적토/반출, 사진 방향·스케일, 기준단면 당일 기록, 안전휀스, 철수 전 점검을 추적한다. |
| 보고서 역산 | 보고서 초록, 도면 목록, 사진대장, 유물·시료 목록은 나중에 따로 만드는 문서가 아니라 현장기록에서 생성되어야 한다. 목차, 원색도판, 일러두기는 보고서 앞에 놓이지만 편집 말미에 원자료 대조 뒤 확정되는 파생 산물이므로 기준 확정·수정 이력도 원자료와 연결해야 한다. 편집방식, 인쇄소 인계, 최종교정, 판형·제본 계획도 어떤 원자료를 보고서 대표 증거로 삼았는지 남기는 판단 기록이다. | `FieldReportConcurrentReview`가 현장 중 원고·도면·사진 가편집본을 대조하고, `ReportPreparationReview`, `ReportStandardHistory`, `ReportEditorialCrossCheck`가 원고, 도면, 사진, 일지, 목록, 편집방식, 원색도판 선정, 인쇄소 인계자료, 기준 변경, 기준토층 사진·도면, 해발고도 대조, 번호 변환표의 불일치를 원기록 ID와 연결한다. |
...
| 행정·보존 이행 | 발굴허가와 보존조치는 결정으로 끝나지 않고 구비서류, 허가조건 이행, 보존의견 출처, 평가자료 요청, 현지·이전·기록보존 방식, 경계도면, 이행 사진, 향후 관리계획, 관리주체, 점검, 재평가로 이어진다. | `ExcavationPermitDocumentSet`, `preservationActionTracking`, `PermitConditionComplianceRecord`, `ReportSubmissionWorkflow`, `StateVestingSelectionRecord`를 조사사업 카드에 연결한다. |
| 의뢰·계획·변경 관리 | 조사기관은 학술 조사뿐 아니라 사업시행자 자료 접수, 선행자료 확인, 현장답사, 조사현황 상시관리, 전문기관 배정, 민원·일정·지장물 조율까지 담당한다. | `InvestigationRequestIntake`와 `InvestigationPlanChangeRecord`가 접수 경로, 기초자료, 계획 전 답사, 접수담당자, 중복·무리한 민원접수 방지, 토지·지장물 제약, 민원 예상사항, 조사기간·예산·조사단 변경, 허가·계약 변경 근거를 저장한다. |
...
| 지표조사 특수기록 | 비문, 암각화, 노출 구조물은 유물산포나 유물실측으로만 처리하면 지표조사 단계의 근거가 흩어진다. | `surfaceSurveySpecialRecord`가 탁본, 간이실측, 전경·근경·세부 사진, 동영상, 3D 기록, 현장지도 연결, 전문가 검토 필요 여부를 Survey 단계에서 남긴다. |
| 질문형 유구 야장 | 유구명만 고르는 방식은 수혈주거지, 고분, 성곽, 생산유구의 조사 과정을 설명하지 못한다. 바닥에서 뜬 유물도 자연유입인지 의도적 폐기인지, 수거 전 실측해야 하는지 현장에서 판단해야 한다. | 유구 유형별 카드에는 단계별 질문, 확인 전제, 대안 해석, 사진·도면·시료 연결을 둔다. 1차 구현은 `Feature`의 수혈건물지 노출·둑, 단계조사 순서, 토층둑 전략, 바닥·시설, 뜬 유물 퇴적 질문, 사용면·최초굴착면 구분, 화재 증거, 중복 선후관계 체크리스트로 시작한다. |
| 구석기 유물 위치·수습 | 구석기 발굴에서는 수습 전 흙기둥 높이, 절대·3차원 좌표, 문화층 번호, 유물 일련번호가 빠지면 분포도와 접합관계도, 문화층별 해석을 되살리기 어렵다. | `Find`의 `paleolithicArtifactPositionRecovery`가 흙기둥 높이, 수습 시점, 수습 전 좌표·문화층·일련번호, 문화층·칸별 보관, 좌표 목록, 분포도와 접합관계도 연결을 유물 카드에서 점검한다. |
...
| 매체의 기록 단위화 | 사진, 도면, 3D, GPS, GIS는 첨부파일이 아니라 판단 근거다. 사진은 좋은 구도보다 조사자 안전이 우선이므로 사다리·고소촬영 위험과 대체수단도 기록해야 하며, 유물사진은 실측도면과 보고서 도판을 검증하는 별도 기록이다. | 매체마다 목적, 대상, 결과·과정 역할, 방향, 기준점, 좌표계, 품질, 후처리, 재촬영·재실측 필요 여부를 가진다. `mediaQualityCheck`는 주기판·방위·스케일 동시촬영, 사진야장·촬영내용 기록, 디지털 메타데이터 대조, 관련 구역 동시 기록, 내부 양방향 촬영, 색·수분 상태, 핵심 단면선, 복수 평면도, 평면·입단면 순서, 추정선·미조사 점선 구분, 장·단축 단면 연결, 산화·환원 색표기뿐 아니라 단계도면 축척·방향·평단면 불일치, 좌우반전·상하반전·뒤집힌 스캔, 저해상도·초점 흔들림·크롭 손상을 함께 점검한다. `Photo`의 `photoCaptureSafetyReview`는 붐대 카메라, 드론, 고소촬영 금지, 사다리 위험, 대체 촬영수단, 안전관리자 확인을 따로 남긴다. `artifactPhotoCaptureStandard`, `artifactPhotoLightingControl`, `artifactPhotoViewPlan`은 실측도면 동일각도, 그레이카드·화이트밸런스, 조명·반사 제어, 정면·측면·내면·굽·접사·질감 사진 계획을 유물사진 카드에 남긴다. |
| 유물실측과 주기 | 유물실측은 사진·탁본·기술과 보완되는 판단 기록이며, 3차원 유물을 2차원 도면으로 줄이기 때문에 기준선, 실측선, 정치 방향, 추정값, 모호값, 주기 항목을 남겨야 한다. 수침목재는 실측 도구 접촉과 작업 중단 보관 상태도 손상 위험이다. | `artifactDrawingPlan`, `artifactDrawingQualityCheck`, `woodenArtifactConditionRecord`가 기준선·실측선 선택 사유, 실측선 이동·연장, 도면으로 표현되지 않는 재질·색조·태토·마모·그을음, 실측자·실측일·축척, 측정 도구 접촉, 눌림·건조수축 위험, 분무·거즈·수조 보관 상태를 기록한다. |
| 원자료 영구보존과 열람 | 야장, 원도면, 원사진, 필름·슬라이드·밀착인화, 보고서 미수록 자료, 실물 유물은 보고서 발간이나 국가귀속 뒤에도 재검토 자료다. 백업 여부와 열람 가능성, 제한 사유가 분리되어야 한다. | `digitalSourcePreservation`이 원야장 원본, 미수록 원도면·원사진, 아날로그 필름, 슬라이드 필름, 밀착인화, 웹·서버 백업, 구매체 상태점검, 실물 유물 열람, 열람신청, 제한 사유, 후속연구자·신진연구자 열람 이력, 두 곳 이상 백업과 보존연한 없음을 함께 추적한다. `mediaQualityCheck`는 사진야장 대조, 촬영내용 기록, 디지털 메타데이터 대조, 인화 사진 주기 기록을 사진 품질 검수에 포함한다. |
| 디지털 조사 품질검수 | 고지형분석, GIS, UAV, LiDAR, 3D 레이저스캔, 사진실측 같은 결과는 장비명과 완성 파일만으로는 검증하기 어렵다. 원천자료가 왜곡·부족하거나, 유적 성격에 맞는 기법 검토와 취득·표현 계획 없이 외주 결과만 받으면 보고서 단계에서 오류를 되짚기 어렵다. | `Operation`의 `digitalSurveyQualityControl`이 원천자료 시점, 원지형 변경 위험, 기준점, 좌표계·지리보정, 오차, 원천자료 한계, 기법 적합성, 취득·표현 계획, 산출물 목적, 외주 결과 인수검수, 최소 분석 위험, 현장 대조와 재분석 가능성을 함께 남긴다. |
...
| 유물·시료 생애주기 | 수습 뒤 목록 작성으로 끝나지 않고 응급처치, 포장, 임시보관, 보존처리, 국가귀속, 이관이 이어진다. 행정귀속·보고서 수록 대상만 유물로 좁히지 않고 탄화곡물, 목탄, 슬래그, 동물뼈, 조개 같은 후속연구 자료도 남긴다. | `artifactHandlingWorkflow`, `artifactTransportSafety`, `temporaryStorageOperation`, `storageEnvironmentControl`이 수습 전 상태부터 이송·임시보관·수장 환경까지 추적하고, `findSampleResearchScope`가 국가귀속 유물, 보고서 수록 유물, 자연과학 시료, 후속연구 후보, 수습·미수습 사유를 `Find`와 `Sample`에서 공통으로 기록한다. |
| 유물 현장복원 연결 | 수거 당시 사진과 세척·접합·복원 과정이 끊기면 토기편의 원개체 후보, 학술자료 후보, 폐기유물 판단 근거가 사라진다. | `Find`의 `artifactFieldRestorationLink`가 수거 사진 묶음, 세척 전 상태, 현장 세척, 접합 후보, 원개체 후보, 복원 참고사진, 복원상태, 학술자료·폐기유물 후보와 전문검토 필요를 같은 유물 기록에 남긴다. |
| 유물 수량과 수장 기준 | 국가귀속·이관용 수량은 파편 수가 아니라 행정 건수, 물리 점수, 동일개체·부속구 판단, 실수량 병기가 결합된 값이다. | `artifactQuantityBasis`가 `건/점/실수량/구성품/근거사진`을 분리하고, `storageEnvironmentControl`이 재질별 온습도·조도·오염물 기준과 실측값을 저장한다. |
| 용어 권위와 검색 분리 | 전문사전 표제어, 기관 관용어, 한자·원어 색인은 모두 필요하지만 저장값으로 바로 합치면 안 된다. 일반 사전의 유적명·수치·색인 원칙도 현장 입력 강제가 아니라 보고서 출력 규칙으로 추적해야 한다. | `TermAuthority`, `TermAlias`, `TermRelationship`을 두고 검색어·보고서출력어·우선표제어·검증상태를 분리한다. `dictionaryEditorialRule`은 한글 표제어, 한자·원어 병기, 법정동·리 기준, 사업명·지정명 찾아가기, 북한식 표기·두음법칙 변형, 분야별 동형 표제어 분리, 유구/유물 단위 출력, 연대·수량 표기, 색인 연결을 표제어 후보에서 따로 관리한다. `termAuthorityStatus`는 사전 자료군 범위 검토, 대량 가져오기 후보·보류, 동일 표제어 충돌, 분야 충돌 검토를 표시해 표제어를 자동 병합하지 않게 한다. |
| 일반 용어와 전문 용어 매핑 | 현장에서는 집자리, 고인돌, 조개더미, 돌방무덤처럼 익숙한 말을 쓰지만 보고서와 전문사전은 주거지, 지석묘, 패총, 석실묘처럼 다른 표제어를 쓰기도 한다. | `TermImportMapping`이 입력어, 권위표제어, 보고서출력어, 원자료 필드·대장, 원문 맥락, 수동 검토 메모, 구조 하위유형, 추정상태, 검증상태를 연결한다. |
...
1. `KoreanFieldwork-*` 공통 값 목록: 기록 생성 시점, 검증상태, 현장기록 품질, 작업 역할·책임, 현장 보호·보안, 제토·둑·안전 관리, 현장시점 누락점검, 최초 노출 기록, 수혈건물지 노출·둑, 수혈건물지 단계조사 순서, 수혈건물지 토층둑 전략, 수혈건물지 바닥·시설, 화재 수혈건물지 증거, 중복 수혈건물지 선후, 지상식 건물 판단, 주혈군 조사·촬영, 초석·기초 흔적 기록, 건물지 전문검토, 건물지 복원 근거, 건물지 탐사·보존 기록, 제철 공정 근거, 제철 노 구조, 제철 부산물 세분, 제철 시료 분석계획, 일일 작업기록, 일지 증거 역할, 일지 검토, 개인 야장 공적기록화, 디지털 원자료 보존, 디지털 조사 품질검수, 정보화 자산 종류, 정보화 자산 관리, 조사 행정 흐름, 허가조건 이행, 보존조치 이행, 조사자료 인계, 용어관계, 용어 검색·매핑, 용어 검증상태, 사전 분야, 용어 적용범위, 출처 우선순위, 지표조사 현장관찰, 지표조사 편향방지, 지표조사 후속조치, 표본조사 적합성, 시굴조사 목적, 시굴 트렌치 설계, 정밀발굴 범위·난이도 근거, GIS·항공사진 예측근거, 예측 현장검증, 유물 관리 절차, 유물 건·점수 산정, 수장환경 관리, 유적 패키지, 형식 논증, 편년 논증, 공반 관계, 해석 논증, 발굴맥락 구성, 발굴행위 역순 검토, 생토·부식토 상대성, 시료 목적, 시료 채취·보관, 식물고고학 시료 설계, 식물유체 표본추출, 플로테이션 처리기록, 식물유체 동정기록, 식물고고학 해석검토, 식물유체 미검출 평가, 매체 증거 역할, 매체 품질검수, 보고서 교차검토, 보고서 평가 환류, 공개자료, 공개·교육 프로그램, 공개등급·민감정보, 매체 권리관리, 해외조사 문화재 리스크, 한국고고학 제도 리스크, 연구 역할 분담, 연구과정 연결상태, 실험고고학 설계, 봉수 물리시설, 봉수 노선·운영, 성곽·봉수 복합관계, 성곽 축조기법 근거, 성벽 기초·기저부 기록, 성곽 보수·정비 기록, 성곽 복원·중건 근거
   ...
   - 수혈건물지 시료 보강 값 목록: 수혈건물지 자연과학 시료 계획
...
`SourceEvidenceIndex`는 `Project` 하위 카드로 두어 제5·6·7·9권과 사전류의 도표, 사진, 표, 캡션, 사례명, 한자, 수치, 출전, 점검표, 분석결과표를 원문 자료 종류, 근거 적용 영역, 원문 대조 상태, 사용 목적과 함께 관리한다. 이 카드는 원문 내용을 저장소에 옮기는 장치가 아니라, OCR 노트에서 뽑은 후보가 성곽·충적지·고분·생산유적·용어 권위 같은 실제 입력 설계로 넘어가기 전에 원PDF 대조 필요 여부와 적용 범위를 남기는 장치다. 인용 정보와 쪽수·도판 위치는 필수값으로 두어, 나중에 값목록이나 도움말을 고칠 때 같은 근거를 다시 찾을 수 있게 한다.

`InformationAsset`은 최종보고서 PDF와 조판 파일만을 위한 카드가 아니다. 조사방법론 자료의 GIS·위치도면·사진 보관 대목을 재대조하면, 조사구역도와 유구배치도는 shapefile 제출 묶음, 좌표계, 기준점, GPS 원시로그, NMEA 궤적까지 보존해야 하고, 고지형 분석과 3D 도면은 지리보정 래스터, 컨트롤 포인트, RMS 오차, UAV 원사진, LiDAR 점군, 사진측량 프로젝트파일, 처리 workflow를 남겨야 재분석이 가능하다. 따라서 `informationAssetType`과 `informationAssetManagement`는 보고서 산출물뿐 아니라 공간·매체 원천자료의 파일경로, 포맷, 좌표계, 처리 과정, 파생 산출물, 제출 묶음, 체크섬, 백업 위치를 추적한다.

`Config-KoreanFieldwork`는 1차 공통 값 목록을 Project, Operation, Survey, FeatureGroup, Feature, FeatureSegment, Find, Sample, Drawing, Photo 화면에 배치한다. 현장기록 품질은 각 기록 단위에, 현장시점 누락점검은 `Feature`, `FeatureSegment`, `Find`, `Sample`에, 최초 노출 기록은 `Feature`와 `FeatureSegment`에 배치했다. `fieldOnlyMissingCheck`와 `firstExposureRecord`는 해당 화면의 필수값으로 지정해 값이 없으면 기존 필수값 경고 흐름에 잡히게 했다. `operationRoleResponsibility`는 `Operation`과 `DailyLog`에 배치해 조사단장, 책임조사원, 현장팀장, 측량·사진·도면·안전·민원·유물정리·시료·일지·보고서 담당과 검토자, 역할 공백을 일지 흐름에서 구조화하게 했다. `siteProtectionSecurity`는 `Operation`에 배치해 허가서 현장 비치, 착수신고 안내, 보안 유지, 주민·관할기관 통보, 유적보호 주의사항, 기상 대비, 야간방범, 임시유물·기록물 보관, 철수 전 미작성자료 조치를 행정 상태가 아니라 당일 실행값으로 남기게 했다. `excavationControlSafety`는 `Operation`에 배치해 구획·둑 계획, 둑 보호, 배수·집수정, 하강조사, 장비 제토 두께, 삽날 관찰, 일일 제토량, 굴삭기 규격·조합, 표토 두께와 토량, 적토장·반출, 생토 위 퇴적두께, 유구 내부토 차이, 작업 안전거리, 기준단면 당일 사진·도면, 안전휀스, 철수 전 기록물 점검을 일지와 함께 관리하게 했다. 수혈건물지·수혈주거지 강의자료에서 반복되는 어깨선, 둑 설정, 단면 보양, 단계별 노출, 동일 촬영지점, 드론 경로, 토층둑·탐색 트렌치 근거, 바닥면·내부시설, 화재 증거, 중복 선후관계는 `pitDwellingExposureBaulk`, `pitDwellingInvestigationSequence`, `pitDwellingSectionStrategy`, `pitDwellingFloorFacility`, `pitDwellingFireEvidence`, `pitDwellingOverlapSequence`로 `Feature`에 배치해 유형명 확정보다 조사 과정의 증거를 먼저 남기게 했다. 제철유적은 `ironProcessEvidence`와 `ironFurnaceStructure`를 `Feature`에, `ironResidueSubtype`을 `Find`에, `ironSampleAnalysisPlan`을 `Sample`에 배치해 노 형태, 송풍·배재 구조, 부산물 세분, 금속학 분석계획을 공정 확정 전에 따로 남기게 했다. 고분은 `tombMoundInvestigation`과 `tombBurialStructureInvestigation`을 `Feature`에 배치해 봉토·분구의 구지표, 성토 단위, 조사둑, 주구, 보존구역과 매장주체부의 묘광, 관·곽·실, 개석, 밀봉토, 폐쇄시설, 추가장·도굴 근거를 나누어 기록하게 했다. 부장품과 제사자료는 `graveGoodsRitualContext`로 `Find`에 배치해 착장, 관 내부·외부, 봉토·주구, 입구·폐쇄부, 의도적 파손, 동물·음식 공헌, 외래요소 후보, 인골과의 위치관계를 유물 카드에서 바로 남기게 했다. 인골은 `humanRemainsRecoveryAnalysis`로 `Sample`에 배치해 전문가 협의, 보존처리 전 DNA, 핀폴 보링 금지, 약한 도구, 차양, 물 사용 제한, 1/5 도면, 포장·주기, 장기건조, 개체식별과 분석 기준을 이어서 기록하게 했다. 일일 작업기록과 일지 검토는 `diaryAbstract`가 있는 `Operation`에 먼저 배치하고, 독립 `DailyLog`는 `Operation` 하위 카드로 두어 당일 사실기록, 누적 조사원·인부·장비 수, 날씨·우천작업, 위원회·전문가 검토회의, 발주처·기관 소통, 분쟁 증거 가능성을 별도 기록하게 했다. 개인 야장 공적기록화는 `Operation`에, 디지털 원자료 보존은 `Project`와 `Operation`에 우선 배치했다. `digitalSourcePreservation`은 원야장 원본, 보고서 미수록 원도면·원사진, 실물 유물 열람, 열람신청, 제한 사유, 후속연구자·신진연구자 열람 이력까지 확장해 백업과 공개 가능성을 분리했다. 독립 `FieldRecordQualityReview`는 `Operation` 하위 카드로 두어 검수 대상 기록, 검수 단계, 수정·보완 근거를 별도 기록하게 했다. 독립 `TermAuthority`는 `FeatureGroup` 하위 카드로 두어 사전 분야, 적용범위, 출처 우선순위, 용어 관계, 검색 매핑, 검증상태를 한 곳에서 관리하게 했다. 독립 `TermAlias`는 `TermAuthority` 하위 카드로 두어 별칭 원문, 현장 동의어, 찾아가기, 기관 관용어, 보고서·사업명, 한자·원어 변형, OCR 변형을 우선표제어와 분리하고, 자동완성 허용·검색어 보존·가져오기 전용·원PDF 대조 필요 같은 처리 방식을 별도 기록하게 했다. 조사 행정 흐름, 허가조건 이행, 보존조치 이행은 `Project`에, 조사자료 인계는 `Project`와 `Operation`에 배치해 표본·시굴·정밀발굴 사이의 기록 이동을 끊기지 않게 했다. 보고서 교차검토와 보고서 평가 환류는 `Project`와 `Operation`에 함께 배치해 원고·도면·사진·목록 대조와 자체평가·위원회평가 차이, 비해당 항목, 보완요구 추적이 조사 단위와 사업 단위에서 모두 남도록 했다. `koreanArchaeologyInstitutionalRisk`는 `Project`에 배치해 국내 구제발굴의 일정 압박, 경쟁입찰, 전문성 약화, 과학분석 배제, 보고서 품질 위험, 기관 역할 불명확이 조사 품질에 미치는 영향을 역할분담·연구과정·보고서 환류와 함께 보게 했다. `Survey`에는 지표조사를 유적 확정 절차가 아니라 현장관찰, 편향방지, 후속조치 판단근거로 관리하는 세 목록과 표본조사 적합성, 시굴조사 목적, 시굴 트렌치 설계, 정밀발굴 범위·난이도 근거, GIS·항공사진 예측근거, 예측 현장검증 목록을 배치했다. `FeatureGroup`, `Feature`, `FeatureSegment`, `Find`에는 용어 관계, 용어 검색·매핑, 사전 근거, OCR 교정 필요, 원PDF 대조 완료, 값목록 후보, UI 노출 보류를 표시하는 용어 검증상태를 배치했다. `Find`에는 `artifactHandlingWorkflow`, `artifactTransportSafety`, `temporaryStorageOperation`, `artifactLabelRegisterLink`, `artifactQuantityBasis`, `storageEnvironmentControl`을 배치해 현장수습, 포장·이송 안전, 임시수장고 운영, 꼬리표·유물대장·정리번호 연결, 응급처치, 건·점수 산정, 동일개체·부속구 판단, 재질별 수장환경을 유물 카드에서 바로 추적하게 했다. `Feature`와 `Find`에는 형식 논증, 편년 논증, 공반 관계, 해석 논증을 함께 배치하고, `FeatureSegment`에는 최소한 편년 논증을 배치해 시대·형식값이 근거 없이 확정되지 않게 했다. Sample 화면에는 시료 목적, 빛·수분·오염·위치도면 연결 조건, 제철 시료 분석계획, 식물유체 표본설계·처리·동정·해석·미검출 평가를 배치했다. Drawing과 Photo 화면에는 `mediaEvidenceRole`, `mediaQualityCheck`, `digitalSourcePreservation`, `reportCrossCheck`를 배치해 방향·축척·기준점·원본 보존·대장번호 대조·보고서 도판 후보 여부를 매체 자체에서 점검하게 했다. 성곽 보강분 중 성문·방어 부속시설, 암문 기능, 여장 세부, 수문·수구, 왜성 해자는 `Feature` 화면의 첫 전문 값 목록으로 옮겼다.

`TermImportMapping`은 `TermAuthority` 하위 카드로 두어 기존 엑셀, 유물대장, 사진 캡션, 보고서 목록의 입력어를 권위 표제어와 보고서 출력어에 연결한다. 이 카드는 자동 치환 규칙이 아니라 원 입력어 보존, 원자료 필드·대장명, 원문 맥락, 권위어 연결, 보고서 출력어 분리, 분야 범위, 수동 검토 필요 여부를 함께 남기는 가져오기 검토 기록이다.

`TermRelationship`도 `TermAuthority` 하위 카드로 두어 단순 동의어가 아닌 용어 관계를 기록한다. 출발 용어, 대상 용어, 관계 근거를 텍스트로 남기고 `partWholeRelation`, `spatialRelation`, 기능·구조 유사성, 분야 제한을 함께 선택한다. 성문-옹성/적대/치성, 토기 공방-수비장/건조장, 수혈건물지-주혈/노지, 석실묘-횡구식·횡혈식 접근 구조처럼 현장 관찰 단위와 보고서 표제어가 관계를 맺지만 하나의 용어로 병합되면 안 되는 경우를 위한 카드다.

`surfaceSurveySpecialRecord`는 지표조사 단계에서만 확보되는 비문·암각화·노출 구조물의 원자료를 Survey에 붙인다. 탁본, 간이실측, 전경·근경·세부 사진, 동영상, 3D 기록, 현장지도 연결, 전문가 검토 필요 여부를 같은 목록으로 두어 이 자료가 유물실측이나 보고서 도판 단계로 흩어지지 않게 한다.

제철유적 보강분은 `IronProcessRelation`을 `Feature` 하위 카드로 두어 이전 공정 산출물과 다음 공정 원료의 연결을 별도 검토 기록으로 남긴다. 이 카드는 환원괴·반환원괴 후보, 단야 원료 후보, 부산물 근거 추정, 분석 근거 추정, 대안 연결 보류, 분석 환류 필요 여부를 공정 확정 전 상태로 추적한다.

수혈건물지 자연과학 시료 보강분은 `Sample`에 `pitDwellingScienceSamplingPlan`을 배치했다. AMS 목탄·목재·패각, 고고지자기 피열토·토기편, 식물규산체 토양·토기편, 부유법 층별·바닥상층 격자토양, 수종분석 목탄·목재, 레플리카·사용흔·전분 갈판·갈봉·태토 기반토·토기·지방산 토양·토기·석기·석재 분석 후보를 채취 시점, 위치도면, 조사종료 후 채취, 강한 세척 회피, 오염위험 검토와 함께 기록한다.
...
편년 단위 보강분은 `Feature`, `FeatureSegment`, `Find`의 `chronologyUnitDecision`에 배치했다. 같은 편년값이라도 유물 형식, 개별 유물, 유구, 유물군, 문화, 층위 중 무엇의 연대인지와 제작·사용·수리/재사용·폐기·매납/퇴적·후퇴적 변형 중 어느 행위 시점인지가 다르므로, 지속시간 위험과 표지유물 편중을 별도 체크로 남긴다.
...
봉수 보강분은 `Feature`에 `beaconPhysicalFacility`, `beaconNetworkOperation`, `fortressBeaconRelation`을 추가했다. 물리시설은 봉수대·봉화대 이칭 확인, 연대, 연조, 봉조, 화덕, 연소실, 외곽석축, 방호벽, 건물지, 훼손·잔존 상태를 기록하고, 노선·운영은 직봉, 간봉, 연변봉수, 내지봉수, 대응봉수, 노선 변경, 관할, 수번 인원, 감관·별장·봉군보 같은 문헌어를 따로 둔다. 산성 정상이나 성문 주변 봉수는 망대 선행, 후대 설치, 공동 사용, 성곽 기능 변화 여부를 판단 보류값과 함께 남긴다.
...
행정·보고서·국가귀속 보강분은 `ExcavationPermitDocumentSet`, `ReportSubmissionWorkflow`, `ReportPreparationReview`, `ReportStandardHistory`, `ReportEditorialCrossCheck`, `InformationAsset`, `StateVestingSelectionRecord`를 `Project` 하위 카드로 두었다. 발굴허가 신청 단계에서는 신청서, 조사계획서, 산출내역, 토지·임야조서, 사업위치도, 건설공사 계획서, 지하 굴착계획, 건축도면, 조경·수목식재계획, 사업자등록증·신분증, 문화재영향검토, 현상변경허가, 위원회 심의와 공정성 침해 여부를 문서 묶음으로 관리한다. 완료신고, 완료자료, 약식보고서, 최종보고서 PDF, 전자행정 접수번호, 초록·협업포털 입력, 원기록 연결은 제출 흐름으로 관리하고, 최종보고서 편집 전에는 일러두기, 허가번호, 방위, 축척·축소율, 해발고도, 토색, 도면기호, 유물 관찰·실측 기준, 원야장·원도면·원사진·대장류·번호 변환표와 도면·사진 일치 여부를 별도 검수한다. 편집 중 원고·도면·사진·목록·기준토층·번호 변환표가 어긋나면 `ReportEditorialCrossCheck`가 불일치와 수정 근거를 원기록 ID로 되돌려 남긴다. `ReportStandardHistory`는 일러두기 기준이 편집 중 확정·변경되는 과정을 따로 남겨 방위·축척·해발고도·토색·도면기호·유구/유물번호·용어 기준이 도면목록·사진대장·유물대장에 함께 반영됐는지 추적한다. `InformationAsset`은 조판 작업파일, 최종보고서 PDF, 표준 전자발굴조사보고서, 도판·도면·사진 DB, 지표조사 GIS, 재래식 매체와 통합 DBMS 연결을 별도 자산으로 남기고, 원기록·대장번호·파일경로·포맷·백업·열람권한·장기보존 위치를 함께 관리한다. 소유권 공고, 선별회의록, 임시보관증, 보관관리기관, 국가귀속대장, 보존처리 후 정정·재평가는 프로젝트 체크값에 묻지 않고 증빙 단위로 추적한다.
...
연접·표형·중복 봉분 선후관계 보강분은 `Feature`에 `tombMoundOverlapSequence`를 추가했다. 관계 확인 트렌치가 중심 트렌치와 달라질 수 있음을 남기고, 매장주체부 축선, 봉분 중심 어긋남, 최신 매장시설 우선 노출, 동일 순서 반복 촬영, 매장시설·봉분 관계, 제사·매납 축조단계, 공유 주구 선후관계를 기록한다. 판단이 닫히지 않으면 `moundFacilityRelationUnresolved`와 `pendingDecision`으로 남겨 보고서 단계에서 무리하게 단일 선후관계로 접지 않게 한다.
...
생활유적 조사절차 보강분은 `Feature`에 `pitFeatureFunctionAssessment`, `pitBuildingLifecycleStage`, `settlementFeatureInvestigationProcedure`, `settlementFeatureTrenchStrategy`를 추가했다. 수혈은 주거지로 바로 확정하지 않고 창고, 공방, 공공시설, 함정, 화장실, 폐기장, 태토 채취장, 노지·경화면·조리용기·저장공 근거를 함께 남긴다. 수혈건물지는 계획, 부지 선정, 수혈 굴착, 지붕 제작, 생활·보수, 폐기, 재사용, 매몰, 가옥장·폐기장 전용 후보까지 생애단계로 남기고, 조사 절차는 사전조사, 평면조사, 중복관계, 트렌치와 둑, 하강, 내부 정리, 바닥조사, 절개와 검토 사진으로 나눈다. 트렌치는 최소 훼손으로 바닥면·벽선·단면 연속성과 도면 역전 위험을 확인하는 전략값으로 관리한다.

취락 단위 공간해석 보강분은 `FeatureGroup`에 `settlementSpatialInterpretation`을 추가했다. 집자리 중복관계, 동시기 집자리, 한 시점 규모, 지붕·벽체·화덕 근거, 행위공간 구획, 공동이용공간과 세대별 공간 후보, 폐기 방식, 바닥 유물분포, 주거지 간 유물 접합 검토, 식물유체와 잔류물·내용물 분석 연결, 반복 축조 원인 검토를 취락 카드에서 함께 남겨 개별 유구 기록이 마을 해석으로 넘어가는 근거를 보존한다.

건물지 주혈·초석 보강분은 `Feature`에 `surfaceBuildingJudgement`, `postholeGroupSurvey`, `foundationTraceRecord`를 추가했다. 지상식·고상식·반고상식·벽주식 판단은 생활면, 주혈 배열, 입지, 목주 잔존, 독립동지주, 회랑·노대·창고·작업장 후보와 분리해 남긴다. 주혈군은 방안실, 함척, 중심못, 색실, 고위·장축·단축·대각·개별 사진, 한 칸 바깥 포함, 유구번호, 직경·깊이, 충전토·다짐, 중심축, 주간거리로 검수한다. 초석·기초는 기둥자리, 먹줄, 탄화흔, 초석설치굴광, 적심석, 보강토, 발취공, 폐기공, 지하식 초석, 토대건물, 기초석, 계단 하부, 지복석, 처마지주, 비계 주공, 조영 기준말뚝, 굴립주·독립기초 구분, 흔적 없음 확인, 보존·처리 결정을 기록한다.

건물지 복원·전문검토 보강분은 `Feature`에 `buildingExpertReview`, `buildingReconstructionEvidence`, `buildingProspectionConservationRecord`를 추가했다. 복원안은 건축사·민속건축·생활사·구조 전문가 검토, 비교 건물, 민족지, 문헌·옛사진, 실물복원·거주실험 참고 여부와 대안 가설을 따로 남긴다. 복원 근거는 정면·측면 칸수, 주간거리, 영조척 후보, 모듈 계산, 표본 수, 오차 범위, 내진·외진, 지붕·벽체·난방·출입 가설, 기와 분포, 목재 부재, 공구흔·수리흔, 시공 변경으로 나누고, 탐사·보존 기록은 사전 사진·지도, 지형·보링, GPR 준비와 한계, 3D 스캔 좌표계·정합오차·원점군 보존, 원위치·이전·복토·복원 보류와 활용 목적을 같은 건물지 기록에서 검토하게 한다.

생산유적과 유물 수습 보강분은 `Feature`와 `Find`에 나누어 배치했다. `productionProcessSystem`은 원료 채취·가공, 수비, 연토, 성형, 재임, 소성, 요출, 선별, 폐기와 보수 폐기물을 공정 순서로 남기고, `productionSiteAssociatedFacility`는 채토장, 점토 저장공, 녹로 축혈, 공방, 수비장, 건조장, 집수·배수시설, 주거지, 숯가마, 폐기장을 생산 체계와 연결한다. `artifactRecoveryPreservationRisk`와 `artifactCleaningDryingControl`은 소형유물·미세박편 유실, 내부토 물체질, 수분·자외선·염·산화·건조수축 위험, 보호장구·전문가 의뢰, 유구 단위 분리, 수습번호 유지, 세척 방식, 수침 보관, 그늘 건조와 급건조 방지를 `Find`에서 관리한다. `artifactHandlingWorkflow`, `artifactTransportSafety`, `temporaryStorageOperation`, `artifactLabelRegisterLink`, `artifactQuantityBasis`, `storageEnvironmentControl`은 현장수습, 포장·이송, 임시수장고 운영, 꼬리표·유물대장·정리번호 연결, 전체 정리·등록·수장 흐름을 담당하고, 가마 내부 유물 귀속 위험은 토기가마 맥락의 `potteryKilnInterpretationRisk`와 함께 본다.
...
지표조사 준비·현장순서 보강분은 `Survey`에 `surfaceSurveyPreparationCheck`, `surfaceSurveyFieldSequence`, `surfaceSurveyMapRequirement`를 추가했다. 조사단 구성, 분야 전문가, 문헌·지도·기조사보고서, 문화유적분포지도, 현황측량도, GPS·사진기·나침반·기록지·유물봉투·탐침봉 같은 준비값을 먼저 점검하고, 현장에서는 경계 GPS, 전경 다방향 사진, 지형별 지구 구분, 기존 문화재, 유물산포, 노출 유구, 절개면 토층, 원지형 훼손, 탐문·설문을 순서대로 남긴다. 탐문·면담은 연락처만 남기지 않고 제보자 역할, 질문지 준비, 구술 지명, 제보가 가능성 정보라는 표시, 보완조사 연결 여부를 `surfaceSurveyFieldDiary`에서 같이 확인한다. 지표조사 지도는 점 하나가 아니라 다각형 조사범위, 유물산포지 곡선 범위, 사업계획도 대조, 축척·도엽번호, DGPS/RTK와 GPS 기준점으로 검증한다. 지표 수습 유물은 `Find`의 `surfaceFindHandlingRecord`에 개별 포장, 유물카드, 지도 표시, GPS 위경도, 지번, 근경·원경 사진, 눈금자·방위판, 부착토 보존과 세척주의를 남긴다.
...
구석기 조사 보강분은 `Survey`에 `paleolithicSurveyStage`, `paleolithicLocationSource`, `paleolithicFieldCollection`, `paleolithicTrialPitCoordinateControl`, `paleolithicProfileSampleRecord`, `paleolithicNonSiteResourceSurvey`를 추가했다. 표면에서 정형 석기가 확인되지 않아도 단구·고토양·기존 신고지점·지도자료, 부착토와 토양시료, 시굴 피트 좌표 유지, 단면 시료, 주변 돌감 분포가 함께 남아야 후속 정밀발굴과 전문검토로 이어질 수 있다. 이어 `Feature`에는 `paleolithicLithicSpatialContext`를 두어 석기 평면분포, 집중부와 산포지, 몸돌·격지·미세박편, 물체질, 접합, 석재별 분포, 자갈돌·탄화물 집중과 활동공간 가설을 발굴 단계에서 남기게 했다. `FeatureSegment`의 `paleolithicCulturalLayerReview`는 자연층과 문화층의 불일치, 기준피트, 토양쐐기, 유물 수직이동·재퇴적·동결융해 교란, 층간 접합, 절대연대 위치를 검토한다. 구석기 꼬리표·유물대장 요구는 모든 출토유물에 적용되는 `Find`의 `artifactLabelRegisterLink`로 이미 승격했으므로, 구석기 전용 묶음은 조사단계와 입지·층위·시료·비유적지 자원, 발굴 중 석기 공간 맥락 기록에 집중한다.
...
경작유구 보강분은 `Feature`에 `cultivationFeatureContext`, `cultivationTrialTrenchStrategy`, `cultivationFeatureEvidence`, `cultivationChronologyAnalysis`, `cultivationIrrigationFacility`, `cultivationPlanInvestigationProcedure`를 배치했다. 입지·맥락에서는 구릉지/충적지 고저, 자연제방·배후습지·곡저부 후보, 경지정리와 과거 항공사진, 관개시설, 물관리 방식을 남기고, 작물명만으로 논·밭을 확정하지 않게 했다. 시굴 트렌치에서는 고랑·이랑 또는 논둑과 직교하는지, 경작면 고저차와 층 경계를 확인했는지, 보조 트렌치·분포 범위·기간·안전을 기록한다. 판정 근거는 논둑·논면, 밭 두둑·고랑, 기능면과 기능 후 퇴적층, 라미나·이질토 블록·단립구조·경작구흔·뿌리흔·식물규산체·산화철망간을 분리한다. 관개시설은 보·저수지·수로·집수시설·물꼬와 말목열·점토보강·부엽공법·누수차단·물흐름 조절을 별도 목록으로 남기고, 평면조사 절차는 시굴 트렌치 복구, 토층 재조사, 노출 즉시 선 확인, 구획조사, 밭 제토 높이와 단면/평면 대조를 한 흐름에서 추적한다. 연대·분석은 경작층 포함 유물, 선별 평면조사, AMS/OSL, 식물규산체·식물유체·토양미세형태·지방산 시료, 주거지·야외노지·수혈과의 관계를 한 흐름에서 추적한다.

시대별 토기 관찰 보강분은 `Find`에 `ceramicTermScope`, `potteryFabricTemperRecord`, `potteryTemperFunctionAssessment`, `potteryProductionLifeRecord`, `potteryFormingTraceAssessment`, `potteryFormingCaution`, `potteryProcessDirectionality`, `potteryProductionOrganizationEvidence`, `potteryComparativeReferenceCheck`, `potteryExperimentalVariableRecord`, `potteryClassificationBasis`, `potteryProvenanceDistributionReview`를 배치했다. 토기·도기·자기·도질토기·스에키 같은 용어 범위는 검색어로 열어두되, 유약 유무, 다공질 태, 소성온도 후보, 원료 점토, 수비, 비짐 재료와 기능, 제작 생애, 성형흔, 조정흔 방향을 유물 카드에서 분리해 남긴다. 특히 실떼기와 회전흔은 물레질 뽑기의 단독 증거로 확정하지 않고, 단일성형 뒤 회전조정, 성형·정형 분리, 기와 성형흔 대조를 함께 확인하게 했다. 생산조직 판단은 가내제작·공방공작·전업성·분업·표준화 후보와 동일 시기·지역·기종 비교, 표본 수, 생활도자·민족지·실험 사례의 한계를 함께 기록해 직접 등치와 성급한 결론을 막는다. 제작실험 변수와 분류 판단근거는 점토 수분·비짐 비율·건조·소성 조건, 땔감 투입량, 소성 분위기 전환, 가마 폐쇄, 수축률, 실패 원인, 기술군·암석기술군·형태기술군 분류를 형식명보다 앞선 근거로 남긴다. `Operation`의 `experimentDesign`은 제작 연쇄, 재료·환경 조건, 건조·소성 일정, 비교 기준, 반복과 실패값을 실험 작업 단위에서 관리한다. 산지와 공방 귀속은 원료 점토·암석학 조성·지질 확인, 공방 후보와 귀속 불확실성, 현지 생산·반입품, 분포 범위와 사용 중 이동, 공급 비중과 교환 방식 질문으로 남겨 한 생산지 결론으로 빨리 닫히지 않게 했다.

토기 소성기술과 토기가마 보강분은 `Find`에 `potteryFiringTraceObservation`, `potteryKilnFurnitureContext`를 배치하고, `Feature`에 `potteryKilnIdentification`, `potteryKilnStructureContext`, `potteryKilnPartInvestigation`, `potteryKilnYardFacility`, `potteryKilnOperationScale`, `potteryKilnInterpretationRisk`를 배치했다. 토기 표면흔은 부정형·원반형·고리형·점열형·사각형·일자형, 위치, 개수, 간격, 함몰, 연소흔·자연유·흑색 윤과의 구분을 유물 카드에서 남긴다. 요도구 관계는 이상재·이기재 기능 후보, 초본류·모래·할석·내화토·점토덩어리·토기편·전용토제품, 중첩소성 조합과 도치 여부를 표면흔과 연결한다. 토기가마는 소성유구/구조요 판정, 요상 경사 단독판정 금지, 화구·연소부·소성부·연도부·회구부의 부위별 조사, 토취장·수비장·연토장·성형장·태토저장·건조장·주거시설군·관리건물·운송로 같은 요장 시설, 가마 수와 동시조업·공급 범위 후보, 최종조업품 자동판정과 회구부 자동연결 금지를 분리해 기록한다. `Sample`에는 `potteryKilnAnalysisPlan`을 두어 숯 AMS·C14, 수종분석, 토기 OSL, 태토 성분분석, 소결 요상·요체 고고지자기 시료를 조사 완료 전 점검하게 했다.

기와가마·와요 보강분은 `Feature`에 `tileKilnStructureContext`, `tileKilnExcavationControl`, `tileKilnPartInvestigation`, `tileKilnOperationSequence`를 배치했다. 요체만 기록하지 않고 요전부, 아궁이, 연소실, 소성실, 연도부, 회구부, 폐기장, 채토공, 제작공방, 건조장, 보관소, 배수시설을 생산공간으로 함께 묶는다. 제토량, 적토장소, 10m 격자 조정, 장축둑과 가마-회구부 연결둑, 최종 주축 수정, 바닥면 평면선, 테스트 트렌치 단면 보정, 구지표·천정 상부·소결 윤곽 보존은 조사 제어 항목으로 분리했다. 부위별 조사에서는 연소실·소성실 분리 근거, 아궁이 폐쇄재와 폐기장 대조, 산화·환원 평단면 색표기, 아궁이·배연부 입체 단면, 연도 미확인 주의, 흡화공 위치·높이·개수, 외부 배연공 후보, 연도부 마감, 배연공 석재 실측, 회구부 관계 검증, 폐기장 범위·트렌치, 사면 폐기장 층위 역전 주의, 재층 일부 보존, 층위별 수습을 남긴다. `Find`에는 `tileKilnFindContext`를 두어 생산품, 불량품, 잔여품, 축조재·요도구·보수재, 폐기 후 유입품, 층위별 문양 기록을 구분하고, `Sample`에는 `tileKilnAnalysisPlan`을 두어 C14 위치별 시료, 고고지자기 다지점 시료, OSL 차광, 기와·가마 태토와 소비지 기와 대조를 한 흐름으로 관리한다.
...
11. 고분·분묘 기록이 묘형명 하나가 아니라 조사 목적, 봉분 트렌치, 성토 세분, 연접·중복 봉분 선후관계, 석곽 벽체, 내부 수습, 묘도·연도 폐쇄, 관대·시상, 제사·매납 위치, 봉토·매장주체부·부장품 위치·인골 수습 기준으로 분리되는가.
12. 성곽 부속시설이 성문·암문·수문·여장·왜성 해자의 이름이 아니라 방어·배수·이동 기능과 잔존·보수 상태로 분리되는가.
13. 봉수 기록이 연대·연소실·건물지 같은 물리 유구와 직봉·대응봉수·감관 같은 노선·운영 정보를 분리하는가.
14. 충적지·경작유구 기록이 표면유물 부재, 색조, 작물명 하나로 확정되지 않고 고지형·토양도·층 개념·분석시료와 함께 검증되는가.
15. 요업유적 기록이 토기·기와·자기 가마 형식명 하나가 아니라 생산공간, 부위별 조사, 출토품 맥락, 분석시료와 소비지 대조로 이어지는가.
16. 보존과학 기록이 실내 후처리로 밀리지 않고 노출·수습·임시보관·분석 의뢰·파괴분석 승인·처리 원칙을 현장 단계에서 추적하는가.
17. 저습지 조사가 현재 습윤 여부 하나가 아니라 항공사진·고지도·보링자료·지형 단계 판독·시굴 위치·극미지형 기록으로 이어지는가.
18. 사진·도면·GPS 기록이 첨부파일 묶음이 아니라 위치품질, 원천자료, 좌표계, 실측 기준, 3D 후처리, 보고서 교차검토를 가진 증거 기록으로 남는가.
19. 디지털 조사·측량 결과가 장비명이나 완성 파일로만 남지 않고 원천자료 시점, 좌표·지리보정, 오차, 원천자료 한계, 기법 적합성, 취득·표현 계획, 산출물 목적, 외주 결과 인수검수, 최소 분석 위험, 현장 대조와 함께 남는가.
20. 측구부탄요와 고고지자기 시료가 낮은 피열흔·구조부위·토층둑·폐쇄석 원위치·시료 방위·결과 품질·복수 편년 대조를 한 흐름으로 남기는가.
21. 일지와 품질검수가 개인 야장, 당일 사실기록, 원매체 보존, 미수록 원자료 열람, 수정 사유, 보고서 평가 환류를 원기록 ID와 함께 추적하는가.
22. 지표조사가 표면 유물 유무만이 아니라 조사 범위, 전문분야, 지도요건, 조사 시점, 증거 부재 근거, 주변 자원, 표면 채집 유물 처리까지 함께 남기는가.
23. 청동기 취락·고인돌·환호·토기 기록이 유형명과 관용어를 바로 확정하지 않고 관찰값, 조사절차, 공반·편년 근거, 대안 해석과 함께 남기는가.
24. 층위와 유구 내부토 기록이 색조·층명 하나로 닫히지 않고 관찰 절차, 층명 체계, 토성 판정, 오인 후보, 축조·사용·폐기·매몰 과정을 함께 남기는가.
25. 생토·부식토·교란토 같은 층명이 절대값으로 저장되지 않고 조사대상시대와 관계유구 기준으로 검토되는가.
26. 식물유체·탄화곡물 시료가 채취 위치, 처리 전 토양량, 물체질·플로테이션, 체눈, 동정 신뢰도, 분석/해석 분리, 미검출 조건을 함께 남기는가.
27. 토기와 신석기 생업 유물이 형식명·어구명 하나로 닫히지 않고 태토·비짐·성형흔·공정방향성·생업 증거 수준을 함께 남기는가.
28. 공개·실험·해외조사·국내 제도 리스크 기록이 홍보 문구나 행사명으로 닫히지 않고 공개등급, 민감정보, 매체 권리, 실험 변수·반복·실패값, 기관 역할, 현지 법제·허가·반출 제한, 구제발굴 일정 압박·과학분석 배제와 연결되는가.
29. 성곽 축조·정비·복원 기록이 판축·복원 같은 결과명으로 닫히지 않고 협판·고정주·달구질흔, 기초·기저부, 붕괴 성돌·해체 단면, 문헌·사진·용척·원부재·복원 제외 근거와 연결되는가.
30. 원문 도표·사진·표·캡션·사례명·한자·수치가 값목록으로 바로 승격되지 않고, `SourceEvidenceIndex`에서 적용 영역, 대조 상태, 사용 목적과 함께 추적되는가.
31. 측량·사진·도면·안전·민원·유물정리·시료·일지·보고서 책임과 검토자가 일지 문장 속에 묻히지 않고 `operationRoleResponsibility`로 구조화되는가.
32. 허가서 비치, 보안, 주민·관할기관 통보, 유적보호 주의사항, 기상 대비, 야간방범, 임시유물·기록물 보관이 행정 메모가 아니라 `Operation`의 `siteProtectionSecurity`로 남는가.
33. 구획·둑·배수·장비 제토·제토량·굴삭기 조합·표토량·적토/반출·기준단면·안전휀스·철수 전 기록물 점검이 일지 문장 속에 묻히지 않고 `Operation`의 `excavationControlSafety`로 검수되는가.
34. 발굴맥락이 유구명 하나로 닫히지 않고 유적·유구·유물·유물복합체·환경 연결, 수평분포·수직층위, 조사방법 선택근거와 함께 남는가.
35. 표토·현대교란·도굴갱·상부퇴적토·유구 매몰토 제거와 축조면·사용면·폐기면 확인이 행위의 역순 원칙으로 검토되는가.
36. 유구·유물 해석이 연구질문, 관찰·해석 분리, 대안해석, 자연·후퇴적 가능성, 분석결과 영향, 보고서 채택 근거와 함께 남는가.
37. 건물지 판단이 지상식·고상식 같은 복원명으로 닫히지 않고 주혈군 측정·촬영, 한 칸 바깥 범위, 초석 설치·발취 흔적, 굴립주·독립기초 구분, 보존 결정을 함께 남기는가.
38. 건물지 복원안이 전문가 검토, 영조척 계산, GPR·3D 후처리, 보존·활용 결정과 분리되어 검증되는가.
39. 수혈건물지 조사가 평면확인, 토층둑 배치, 1~3차 노출, 동일 촬영지점·드론 경로, 외부시설 조사와 해체 후 최종평면으로 이어지는가.

`term-authority-alias-sample.json`은 별칭뿐 아니라 가져오기 입력어와 보고서 출력어가 권위 표제어 저장값으로 섞이지 않는지도 함께 확인한다.

`building-site-posthole-foundation-sample.json`은 지상식·고상식 건물 후보, 주혈군 촬영·측정, 사라진 초석과 기초 흔적이 복원명 확정 전 단계에서 검증 가능한 관찰값으로 남는지 확인한다.
같은 샘플은 건축사·민속건축 검토, 영조척 후보와 오차 범위, 3D 정합오차, 원점군 보존, 복원 보류와 활용 목적도 함께 남기는지 확인한다.
`bronze-age-settlement-dolmen-workflow-sample.json`은 수혈건물지 후보가 단계별 노출, 동일 촬영지점·드론 경로, 토층둑·탐색 트렌치 설정 근거까지 갖춘 뒤 유형명 후보와 연결되는지 확인한다.
`tomb-burial-workflow-sample.json`은 연접·중복 봉분의 관계 확인 트렌치, 최신 매장시설 우선 노출, 동일 순서 반복 촬영, 공유 주구 선후관계가 봉토·매장주체부 기록과 함께 남는지도 확인한다.

2026-06-19 검증 샘플은 `docs/korean-fieldwork/samples`에 둔다. `field-record-preservation-sample.json`은 수혈건물지 후보의 최초 노출, 내부층, 유물, 시료, 사진, 도면, 소형 시설 도면화, 바닥 하부 주혈 확인이 같은 작업기록으로 이어지는지 확인하고, `media-drawing-gps-workflow-sample.json`은 GPS/NMEA, 디지털 조사 품질검수, 사진 위치연동, 고지도·분포도 작성, 유물실측, 수침목재 습식 실측, 3D 원천자료 보존, 실내 보완 도면과 3D 산출물의 조사자 재검수가 같은 매체 증거 흐름으로 검수되는지 확인한다. `daily-log-quality-review-workflow-sample.json`은 개인 야장 공적기록화, 당일 작업일지, 작업 역할·책임, 제토·둑·안전 관리, 사진·도면·유물·시료 목록 대조, 수정 사유, 원기록 보존, 보고서 평가 환류가 독립 `DailyLog`와 `FieldRecordQualityReview` 카드로 이어지는지 확인한다. `stage-transition-handover-sample.json`은 지표조사·표본조사·시굴조사에서 정밀발굴 전환과 후속기관 인계가 이어지는지 확인한다. `administrative-workflow-sample.json`은 조사의뢰 접수, 발굴허가 신청 구비서류, 계획 변경, 현장 보호·보안, 전문가 검토회의, 부분완료, 보고서 작성 전 일러두기·원자료 대조, 지표조사 결과 처리, 자료 인수인계가 실제 값목록으로 표현되는지 확인한다. `surface-survey-scope-absence-sample.json`은 지표조사 범위·전문분야·지도요건·시점 검토, 표면 증거 부재 근거, 비유적 자원조사, 표면 채집 유물 처리가 하나의 후속조사 판단 흐름으로 이어지는지 확인한다. `artifact-label-register-sample.json`은 현장 꼬리표, 유물대장, 고유 등록번호, 정리번호, 건·점수 산정과 부속구 수량 근거가 같은 `Find` 기록에서 이어지는지 확인한다. `term-authority-alias-sample.json`은 현장 관용어와 권위 표제어가 같은 저장값으로 섞이지 않는지 확인한다. `faunal-recovery-quantification-sample.json`은 패총 동물유체 시료가 물체질, 블록시료 치수, 보존 취급, 동정 불확실성, NISP·MNE·MNI·MAU·%MAU 한계와 함께 남는지 확인한다. `archaeobotany-flotation-workflow-sample.json`은 식물유체·탄화곡물 시료가 채취 맥락, 처리 전 토양량, 물체질·플로테이션, 체눈, 동정 근거, 분석/해석 분리, 미검출 평가와 함께 남는지 확인한다. `pottery-technology-subsistence-workflow-sample.json`은 토기편과 신석기 생업 유물이 용어 범위, 태토·비짐, 비짐 기능, 제작 생애, 성형흔, 공정 방향성, 생산조직 근거, 비교표본 검토, 산지·분포 검토, 어구·해양자원 증거 수준을 분리하는지 확인한다. `iron-production-workflow-sample.json`은 제철유적의 노 구조, 부산물 세분, 파괴분석 승인, 금속학 분석계획이 공정 확정 전에 함께 남는지 확인한다. `bronze-age-settlement-dolmen-workflow-sample.json`은 청동기 수혈건물지, 고인돌·지석묘, 환호, 무문토기·민무늬토기 기록이 관찰값·조사절차·권위어·공반·편년 근거를 분리하는지 확인한다. `stratigraphy-feature-lifecycle-workflow-sample.json`은 층위 관찰 절차, 층명 체계, 토성 판정, 내부토 해석, 이질토 블록, 매몰 과정, 색조·후성변형 오인 방지와 발굴맥락 구성이 `Feature`와 `FeatureSegment` 기록으로 남는지 확인한다. `tomb-burial-workflow-sample.json`은 고분·분묘의 조사 목적, 봉분 트렌치, 성토 세분, 석곽 벽체, 내부 수습, 석실 묘형 보류, 묘도·연도 폐쇄, 관대·시상 사용, 제사·매납 위치, 봉토·분구, 매장주체부, 부장품 위치 맥락, 인골 수습 기준이 묘형명 하나로 접히지 않는지 확인한다. `fortification-facility-workflow-sample.json`은 성곽의 성문·암문·여장, 수문·은구, 왜성 해자가 부재·기능·잔존 상태로 분리되는지 확인한다. `fortification-construction-restoration-sample.json`은 판축 후보 성벽 단면, 기초·기저부, 보수·정비, 복원·중건 근거가 같은 성곽 기록에서 이어지는지 확인한다. `alluvial-landform-layer-workflow-sample.json`은 충적지 지표조사, 토양도 예측, a+b층·검출면 판정, 경작유구와 고환경 시료가 표면유물 부재나 작물명 하나로 접히지 않는지 확인한다. `wetland-survey-microtopography-sample.json`은 저습지 지형판독이 항공사진·고지도·보링자료, 고하천·자연제방·단구면·배후습지 표시, 시굴 위치, 매몰 수전·1회성 퇴적·미화석 시료로 이어지는지 확인한다. `ceramic-kiln-production-workflow-sample.json`은 토기가마·기와가마·자기요장이 생산공간, 부위별 조사, 출토품 맥락, 분석계획, 소비지 비교로 이어지는지 확인한다. `charcoal-kiln-archaeomagnetic-workflow-sample.json`은 측구부탄요가 낮은 피열흔·측구·폐쇄석·토층둑·목탄시료와 고고지자기 정방위 시료, 방위 보정, 결과 품질, 복수 편년 대조로 이어지는지 확인한다. `conservation-science-fieldwork-sample.json`은 수침목재·칠기·금속·토도류·지류·직물·인골·토양 시료가 현장 수습과 분석·보존처리 결정으로 이어지는지 확인한다. `public-research-governance-sample.json`은 공개 후보 사진·3D 도면, 공개등급, 매체 권리, 현장설명회, 실험 설계, 해외조사 리스크, 국내 제도 리스크, 연구 역할 분담이 같은 사업·작업기록에서 이어지는지 확인한다. `source-evidence-index-sample.json`은 도표·사진·표·캡션·사례명·한자·수치가 `SourceEvidenceIndex`에서 원문 대조 상태와 사용 목적을 가진 채 남는지 확인한다. 이 샘플들은 `korean-fieldwork-configuration.spec.ts`에서 실제 폼과 값목록 키를 쓰는지 검증한다.
