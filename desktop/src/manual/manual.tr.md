...
* *Tipi Tara*: Girdiyi, kamera taraması yoluyla tipinin QR kodunu okuyarak bir tipe bağlar (yalnızca "Buluntu" kategorisi ve alt kategorilerindeki girdiler için geçerlidir)
...
Lütfen unutmayın; QR kodları yalnızca "Buluntu", "Buluntu Koleksiyonu", "Numune", "Tip" ve "Depolama yeri" kategorileri ile bunların alt kategorileri için kullanılabilecektir.
...
#### Tarihleme
Bir veya daha fazla tarihlemenin belirtilmesi. Olası tarihleme tipleri şunlardır: Dönem, Tek yıl, Önce, Sonra, Bilimsel.
...
Geometri oluşturulabilen her kategoriye ait formda her zaman "Geometri" giriş türünde bir alan bulunur. Alan düzenleyicisinde, bu alan için izin verilen geometri tiplerini seçebilirsiniz. İlgili kategoriye ait girdiler için geometri oluştururken, yalnızca seçilen geometri tipleri seçilebilir durumdadır. Varsayılan olarak, tüm geometri tiplerine izin verilir.

Field Desktop aşağıdaki geometri tipini destekler:
...
* *type*: Tarihleme tipi. Olası değerler şunlardır: *range* (Dönem), *single* (Tek yıl), *before* (Önce), *after* (Sonra), *scientific* (Bilimsel)
* *begin*: Tarihleme tipi *after* ve *range* için için başlangıç ​​tarihi olarak belirlenen yıl.
* *end*: Tarihleme tip *single*, *before*, *scientific* ve *range* için bitiş tarihi olarak belirlenen yıl.
* *margin*: Tarihleme tipi *scientific* için yıl cinsinden tolerans aralığı
* *source*: Tarihlemenin kaynağı, çok dilli metin alanı
* *isImprecise*: "Hassas olmayan". *scientific* (Bilimsel) tarihleme tipi için ayarlanamaz. Olası değerler şunlardır: *true* (evet), *false* (hayır)
* *isUncertain*: "Belirsiz". *scientific* (Bilimsel) tarihleme tipi için ayarlanamaz. Olası değerler şunlardır: *true* (evet), *false* (hayır)
...
Bir GeoJSON dosyası her zaman en üst düzeyde "FeatureCollection" tipinde bir nesne içermelidir. Bu nesne de sırayla "Feature" tipinde tekil nesneler içerir.
...
"Feature" tipindeki her nesne, Field'daki bir girdiye ve ilişkili geometriye karşılık gelir. Bir feature her zaman *geometry* ve *properties* alanlarını içerir: *geometry* alanı geometri verilerini içerirken, *properties* alanındaki veriler Field'daki girdiye bağlantı kurar.


##### Geometri

Aşağıdaki geometri tipleri desteklenmektedir:
...
Bir Shapefile yalnızca tek tipte geometrileri içerebildiğinden, Field Desktop'tan dışa aktarırken her bir geometri tipi için toplam üç Shapefile (her biri yukarıda belirtilen dört veya beş dosyadan oluşur) oluşturulur. Tekil tipler, Shapefile'da birden fazla tip olarak kaydedilir ve aşağıdaki tipte dosyalar üretilir:
...
* *type*: Tarihleme tipi. Olası değerler şunlardır: *range* (Dönem), *single* (Tek yıl), *before* (Önce), *after* (Sonra), *scientific* (Bilimsel)
* *begin*: Tarihleme tipleri *after* ve *range* için başlangıç ​​tarihi olarak belirlenen yıl.
* *end*: Tarihleme tipleri *single*, *before*, *scientific* ve *range* için bitiş tarihi olarak belirlenen yıl.
* *margin*: Tarihleme tipi *scientific* için yıl cinsinden tolerans aralığı
* *source*: Tarihlemenin kaynağı, çok dilli metin alanı
* *isImprecise*: "Hassas olmayan". *scientific* (Bilimsel) tarihleme tipi için ayarlanamaz. Olası değerler şunlardır: *true* (evet), *false* (hayır)
* *isUncertain*: "Belirsiz". *scientific* (Bilimsel) tarihleme tipi için ayarlanamaz. Olası değerler şunlardır: *true* (evet), *false* (hayır)
...
Bir tip kataloğu, tüm tipleriyle ve bağlantılı görselleriyle birlikte dışa aktarılır. Bağlantılı buluntular **dışa aktarılmaz**.

Katalog dosyası başka bir projeye içe aktarıldıktan sonra, tip kataloğu, tip yönetiminde her zamanki gibi görüntülenebilir ve buluntu tanımlaması için kullanılabilir. Başka bir projeden içe aktarılan bir kataloğu düzenlemek **mümkün değildir**: İçe aktarılan tip katalogları ve bunların bağlantılı görselleri projeden bir bütün olarak silinebilir, ancak düzenlenemez, genişletilemez veya (yalnızca belirli bazı tipler veya bağlantılı görseller) kısmen silinemez. Ancak, yeni bir katalog dosyası içe aktarılarak tip kataloğunun güncellenmiş bir sürümle değiştirilmesi mümkündür. Buluntulara yönelik önceden oluşturulmuş tüm bağlantılar korunur. Bu kısıtlama, dışa aktarmadan sonra daha sonraki bir tarihte aynı projeye geri aktarılan tip katalogları için de geçerli değildir. Bu durumlarda, katalog tamamen düzenlenebilir ve genişletilebilir.
...
* *Katalog*: Buradan dışa aktarılacak katalog tipini seçin. Bu seçenek yalnızca şu anda açık olan projede katalog tipleri varsa görüntülenir. Diğer projelerden içe aktarılan kataloglar seçilemez.
...
### İzin verilmeyen geometri tipi
Girdinin geometrisi, ilgili kategori için izin verilen bir geometri tipine uymuyor.

#### Olası nedenler
* Geometri girdiye eklendikten sonra, ilgili kategori için izin verilen geometri tipi seçimi kaldırıldı.

#### Olası çözümler
* *Düzenle* Butonu: Geometriyi ayarlamak ve izin verilen bir geometri tipi seçmek için kaynak düzenleyiciyi açın.
* Yapılandırma düzenleyicisinde ilgili kategorinin *Geometri* alanını düzenleyin ve girdinin geometri tipini izin verilen bir geometri tipi olarak seçin.
...
Bu API uç noktası, uygulamada açık olan projeye veri aktarmak için kullanılabilir. "Araçlar" ➝ "İçe Aktar" menüsünden erişilebilen içe aktarma aracıyla aynı işleve sahiptir. Seçenekler ve biçimler hakkında ayrıntılı açıklamalar "İçe Aktar ve Dışa Aktar" bölümünde bulunabilir.

İçe aktarılacak veriler istek gövdesine eklenmelidir.

Parametreler:
* *format*: İçe aktarılacak verilerin biçimi. Desteklenen biçimler: *csv*, *geojson*, *jsonl*
...
Bu API uç noktası, uygulamada açık olan projeden veri dışa aktarmak için kullanılabilir. "Araçlar" ➝ "Dışa Aktar" menüsünden erişilebilen dışa aktarma aracıyla aynı işleve sahiptir. Seçenekler ve biçimler hakkında ayrıntılı açıklamalar "İçe ve Dışa Aktar" bölümünde bulunabilir.

Parametreler:
* *format*: Verilerin dışa aktarılacağı format. Desteklenen formatlar: *csv*, *geojson*
