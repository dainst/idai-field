...
2026-06-20 수혈주거지 조사법 색인도 다시 확인했다. `발굴조사_업무의_이해_교재_(2).pdf`는 PDF 파일쪽과 인쇄쪽이 어긋나 `method19_pit_dwellings_pages205_258/page_214.png`가 인쇄 206쪽 단계별 사진 도판임을 확인했고, `page_215.png`, `page_220.png`로 트렌치·토층둑과 바닥 조사 슬라이드를 대조했다. `현장조사_방법과_해석.pdf`는 `method30_pitdwelling_pages119_143_rotated/page_119.png`, `page_123.png`, `page_126.png`, `page_127.png`, `page_128.png`로 장 표지, 매몰양상, 토층둑 설정, 유구·유물 사진촬영, 분층기재·유물 높이 슬라이드를 확인했다. OCR 텍스트는 여전히 오독이 많으므로 `ocrCorrectionNeeded`는 남기되, 핵심 캡션은 직접 PDF 대조가 끝난 `candidate` 색인으로 조정했다. 이번 확인분은 `FeatureSegment.pitDwellingStratifiedFillRecord`와 `stratigraphy-feature-lifecycle-workflow-sample.json`에 반영했다.
