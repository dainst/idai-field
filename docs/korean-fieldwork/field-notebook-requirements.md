...
| 현장기록 품질검수 | 좋은 야장은 개인 메모가 아니라 즉시성, 사실/해석 분리, 교차검토가 가능한 공적 기록이다. 같은 유형의 유구라도 각각을 별개 질문으로 보고, 현장 조사자의 유구별 서사가 보고서 기술까지 이어져야 한다. | `FieldRecordQualityReview`를 두고 작성자, 담당 유구, 작성시점, 검토자, 사실/해석 분리, 수정 근거를 저장한다. `fieldRecordQuality`는 유구별 서사 기록, 현장-보고서 연속성, 조사자 보고서기술 연결을 함께 점검한다. |
