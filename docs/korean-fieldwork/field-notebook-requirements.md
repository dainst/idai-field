...
`TermRelationship`도 `TermAuthority` 하위 카드로 두어 단순 동의어가 아닌 용어 관계를 기록한다. 출발 용어, 대상 용어, 관계 근거를 텍스트로 남기고 `partWholeRelation`, `spatialRelation`, 기능·구조 유사성, 분야 제한을 함께 선택한다. 성문-옹성/적대/치성, 토기 공방-수비장/건조장, 수혈건물지-주혈/노지처럼 현장 관찰 단위와 보고서 표제어가 관계를 맺지만 하나의 용어로 병합되면 안 되는 경우를 위한 카드다.
