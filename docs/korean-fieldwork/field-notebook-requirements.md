...
행정·보고서·국가귀속 보강분은 `ExcavationPermitDocumentSet`, `ReportSubmissionWorkflow`, `ReportPreparationReview`, `StateVestingSelectionRecord`를 `Project` 하위 카드로 두었다. 발굴허가 신청 단계에서는 신청서, 조사계획서, 산출내역, 토지·임야조서, 사업위치도, 건설공사 계획서, 지하 굴착계획, 건축도면, 조경·수목식재계획, 사업자등록증·신분증, 문화재영향검토, 현상변경허가, 위원회 심의와 공정성 침해 여부를 문서 묶음으로 관리한다. 완료신고, 완료자료, 약식보고서, 최종보고서 PDF, 전자행정 접수번호, 초록·협업포털 입력, 원기록 연결은 제출 흐름으로 관리하고, 최종보고서 편집 전에는 일러두기, 허가번호, 방위, 축척·축소율, 해발고도, 토색, 도면기호, 유물 관찰·실측 기준, 원야장·원도면·원사진·대장류·번호 변환표와 도면·사진 일치 여부를 별도 검수한다. 소유권 공고, 선별회의록, 임시보관증, 보관관리기관, 국가귀속대장, 보존처리 후 정정·재평가는 프로젝트 체크값에 묻지 않고 증빙 단위로 추적한다.
