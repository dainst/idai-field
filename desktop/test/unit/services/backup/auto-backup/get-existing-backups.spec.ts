...
describe('get existing backups', () => {
    ...
    test('build backups map', () => {
        ...
        const backups: BackupsMap = getExistingBackups(backupDirectoryPath, true);
    ...
    test('return empty map if backup directory does not exist', () => {
        
        const backupDirectoryPath: string = process.cwd() + '/test/test-data/non-existing-backups';
