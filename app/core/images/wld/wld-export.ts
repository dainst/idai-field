...
function getWldFileContent(imageDoc: ImageDocument): string {
    ...
    const width = imageDoc.resource.width;
    const height = imageDoc.resource.height;
    ...
    lines[4] = georef.topRightCoordinates[1];
    lines[5] = georef.topRightCoordinates[0];
    return lines.map((x: number) => x.toFixed(8)).join('\n');
}
