...
| 행정·보존 이행 | 발굴허가와 보존조치는 결정으로 끝나지 않고 구비서류, 허가조건 이행, 통보, 관리주체, 점검, 재평가로 이어진다. | `ExcavationPermitDocumentSet`, `PreservationDecisionPackage`, `PermitConditionComplianceRecord`, `ReportSubmissionWorkflow`, `StateVestingSelectionRecord`를 조사사업 카드에 연결한다. |
