...
한국 고고학 현장에서 태블릿과 데스크톱으로 함께 쓰는 디지털 야장입니다. 현장에서는 위치를 찍고, 사진을 붙이고, 펜으로 메모하고, 유구와 트렌치 기록을 빠르게 남깁니다. 사무실에서는 그 기록을 지도, 드론사진, SHP/DXF/GeoJSON, CAD 성과물, 사진대장, 보고서 자료와 맞춰 정리합니다.
...
디지털 야장은 표본조사, 시굴조사, 발굴조사에서 반복되는 현장 기록을 놓치지 않기 위한 도구입니다. 조사구역, 트렌치, 유구, 피트, 토층, 사진, 유물, 시료, 실측 자료를 서로 연결해 두면 보고서와 도면 정리 단계에서 원기록을 다시 찾기 쉽습니다.

이 앱은 현장에서 모든 판단을 끝내라고 요구하지 않습니다. 유구의 성격이나 시기는 조사 중에 바뀔 수 있으므로, 처음에는 위치와 사진, 메모를 빠르게 남기고 필요할 때 유구 성격, 시기, 토층, 유물, 보완 메모를 더해 가는 흐름을 목표로 합니다.
...
| 원칙 | 의미 |
| --- | --- |
| 작게 시작 | 유구를 추가할 때는 위치, 임시 번호, 사진, 메모처럼 바로 필요한 것부터 남깁니다. |
| 나중에 보완 | 유구 성격과 시기는 조사 중 알게 되면 선택하고, 이후에도 수정할 수 있게 합니다. |
| 흐름 안내 | 발굴조사처럼 기록할 것이 많은 경우에는 유구 성격에 맞춰 필요한 항목을 안내합니다. |
| 판단 보존 | 처음 판단과 나중 수정이 끊기지 않도록 기록을 같은 유구 안에 이어 붙입니다. |
...
발굴조사는 유구 단위로 기록합니다. 제토 중 확인된 유구의 위치를 먼저 찍고, 조사 전 사진을 남긴 뒤 조사 중 사진, 토층 사진, 유물 노출 사진, 유물 수습, 완료 사진, 실측 여부를 이어서 기록합니다. 처음에는 `미상`이나 `추정`으로 두었다가, 조사하면서 유구 성격과 시기를 고쳐 갈 수 있어야 합니다.
...
토큰은 저장소에 커밋하지 않습니다. 이미 GitHub 보안 경고에 잡힌 기존 토큰은 코드에서 제거했지만, 공개 이력에 노출된 값이므로 Mapbox에서 폐기해야 합니다.

## 📱 태블릿과 데스크톱

태블릿 전용 데이터 구조를 따로 만들지 않고, 원본 iDAI.field가 쓰는 문서와 geometry 흐름을 유지합니다. 태블릿에서 만든 조사경계와 유구 위치는 데스크톱에서도 같은 프로젝트 데이터로 읽히며, 데스크톱에서 정리한 위치 보정도 같은 기록에 이어집니다.
